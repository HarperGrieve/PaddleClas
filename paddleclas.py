...
class Classifier(object):
    def __init__(self, training_config_path: str, training_config_dir: str):
        self.inference_config = cfg.get_config(training_config_path, show=False)
        self.inference_config["Global"]["pretrained_model"] = training_config_dir
        self.engine = Engine(self.inference_config, mode="infer")

    def infer(self, img):
        return self.engine.image_infer(img)


class Trainer(object):
    def __init__(self, training_config_path: str):
        self.training_config = cfg.get_config(training_config_path, show=False)

    def train(self) -> None:
        self.training_config.profiler_options = None
        engine = Engine(self.training_config, mode="train")
        engine.train()
        return

    def export(self, modelDir, destDir) -> None:
        self.training_config["Global"]["pretrained_model"] = modelDir
        # self.training_config.save_inference_dir = destDir
        engine = Engine(self.training_config, mode="export")
        engine.export()
